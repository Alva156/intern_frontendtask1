...
function UserCompanies() {
  ...
  const [appliedCompanies, setAppliedCompanies] = useState([]); // Stores applied companies
  ...
  const handleApplyClick = (company) => {
    setAppliedCompanies((prev) => {
      if (!prev.some((c) => c.id === company.id)) {
        return [...prev, company]; // Add only if not already applied
      }
      return prev; // If already applied, keep the list unchanged
    });
    setShowPopup(true);
    setSelectedCompany(null);
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      {appliedCompanies.length > 0 && (
        <div className="mb-6 p-4 bg-gray-100 rounded-md">
          <h2 className="text-xl font-semibold mb-2">Applied Companies:</h2>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          ...
          >
            ...
            <button
              ...
              onClick={() => handleApplyClick(selectedCompany)}
            >
              Apply Now
            </button>
          </div>
